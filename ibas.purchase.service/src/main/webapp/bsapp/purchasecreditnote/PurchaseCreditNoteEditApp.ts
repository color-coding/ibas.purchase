...
namespace purchase {
    export namespace app {
        ...
        export class PurchaseCreditNoteEditApp extends ibas.BOEditService<IPurchaseCreditNoteEditView, bo.PurchaseCreditNote> {
            ...
            /** 选择采购贷项客户事件 */
            private choosePurchaseCreditNoteSupplier(): void {
                    ...
                    this.messages({
                        ...
                        onCompleted: (action) => {
                            if (action === ibas.emMessageAction.YES) {
                                ...
                                this.choosePurchaseCreditNoteSupplier();
                            }
                        }
                    });
                    return;
            ...
            /** 更改行价格 */
            private changePurchaseCreditNoteItemPrice(priceList: number | ibas.Criteria): void {
                if (typeof priceList === "number" && priceList > 0) {
                    ...
                    if (config.get(config.CONFIG_ITEM_FORCE_UPDATE_PRICE_FOR_PRICE_LIST_CHANGED, true) === true) {
                        // 强制刷新价格
                        this.changePurchaseCreditNoteItemPrice(criteria);
                    } else {
                        this.messages({
                            type: ibas.emMessageType.QUESTION,
                    ...
                    boRepository.fetchMaterialPrice({
                        ...
                        onCompleted: (opRslt) => {
                            for (let item of opRslt.resultObjects) {
                                this.editData.purchaseCreditNoteItems.forEach((value) => {
            ...
            /** 选择采购贷项行物料事件 */
            private choosePurchaseCreditNoteItemMaterial(caller: bo.PurchaseCreditNoteItem): void {
                let that: this = this;
            ...
            /** 选择采购贷项行仓库事件 */
            private choosePurchaseCreditNoteItemWarehouse(caller: bo.PurchaseCreditNoteItem): void {
                let that: this = this;
