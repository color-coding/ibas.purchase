    ...
    export namespace app {
        ...
        export class PurchaseQuoteEditApp extends ibas.BOEditService<IPurchaseQuoteEditView, bo.PurchaseQuote> {
            ...
            protected registerView(): void {
                ...
                this.view.choosePurchaseQuoteItemMaterialVersionEvent = this.choosePurchaseQuoteItemMaterialVersion;
                this.view.choosePurchaseQuoteItemMaterialCatalogEvent = this.choosePurchaseQuoteItemMaterialCatalog;
            ...
            protected createData(clone: boolean | Blob): void {
                ...
                let createData: Function = function (): void {
                    if (clone instanceof Blob) {
                        ...
                        boRepository.parse<bo.PurchaseQuote>({
            ...
            /** 更改行价格 */
            private changePurchaseQuoteItemPrice(priceList: number | ibas.ICriteria, items?: bo.PurchaseQuoteItem[]): void {
                if (ibas.objects.isNull(items)) {
                    items = this.editData.purchaseQuoteItems.filterDeleted();
                }
                if (typeof priceList === "number" && ibas.numbers.valueOf(priceList) !== 0) {
                    ...
                    if (config.get(config.CONFIG_ITEM_FORCE_UPDATE_PRICE_FOR_PRICE_LIST_CHANGED, true) === true) {
                        // 强制刷新价格
                        this.changePurchaseQuoteItemPrice(criteria, items);
                    } else {
                        this.messages({
                            type: ibas.emMessageType.QUESTION,
                    ...
                    onCompleted(selecteds: ibas.IList<materials.bo.IProduct>): void {
                        ...
                        for (let selected of selecteds) {
                            ...
                            beChangeds.add({
                                ...
                                setUnitRate(this: bo.PurchaseQuoteItem, value: number): void {
                                    this.uomRate = value;
                                }
                            });
                            item = null;
            ...
            /** 添加采购报价-行事件 */
            private addPurchaseQuoteItem(items: bo.PurchaseQuoteItem[] | number): void {
            ...
            /** 选择一揽子协议事件 */
            private choosePurchaseQuoteBlanketAgreement(): void {
                if (ibas.objects.isNull(this.editData) || ibas.strings.isEmpty(this.editData.supplierCode)) {
                    this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_please_chooose_data",
                        ibas.i18n.prop("bo_purchasequote_suppliercode")
                    ));
                                ...
                                onCompleted: (opRsltPRD) => {
                                    ...
                                    for (let selected of selecteds) {
                                        ...
                                        for (let baItem of selected.blanketAgreementItems) {
                                            ...
                                            if (that.editData.purchaseQuoteItems.firstOrDefault(
                                                c => c.baseDocumentType === baItem.objectCode
                                                    && c.baseDocumentEntry === baItem.docEntry
                                                    && c.baseDocumentLineId === baItem.lineId) !== null) {
                                                continue;
                                            }
                                            let item: bo.PurchaseQuoteItem = that.editData.purchaseQuoteItems.create();
                ...
                if (ibas.objects.isNull(criteria)) {
                    ...
                    ibas.servicesManager.runChooseService<materials.bo.IUnit>({
                        ...
                        onCompleted(selecteds: ibas.IList<materials.bo.IUnit>): void {
                            ...
                            that.changePurchaseQuoteItemPrice(that.editData.priceList, [caller]);
                        }
                    });
                }
            }
            ...
            private choosePurchaseQuoteItemMaterialVersion(caller: bo.PurchaseQuoteItem): void {
                let criteria: ibas.ICriteria = new ibas.Criteria();
                let condition: ibas.ICondition = criteria.conditions.create();
            ...
            protected choosePurchaseQuoteItemMaterialCatalog(caller: bo.PurchaseQuoteItem, filterConditions?: ibas.ICondition[]): void {
                ...
                ibas.servicesManager.runChooseService<materials.bo.BusinessPartnerMaterialCatalog>({
                    ...
                    onCompleted: (selecteds) => {
                        let count: number = this.editData.purchaseQuoteItems.length;
                        for (let selected of selecteds) {
                            if (ibas.strings.isEmpty(selected.itemCode)) {
                                continue;
                            }
                            if (ibas.objects.isNull(caller)) {
                                caller = this.editData.purchaseQuoteItems.create();
                            }
                            caller.catalogCode = selected.catalogCode;
                            condition = new ibas.Condition();
                            condition.alias = materials.bo.Material.PROPERTY_CODE_NAME;
                            condition.value = selected.itemCode;
                            this.choosePurchaseQuoteItemMaterial(caller, [condition]);
                            caller = null;
                        }
                        if (this.editData.purchaseQuoteItems.length > count) {
                            this.view.showPurchaseQuoteItems(this.editData.purchaseQuoteItems.filterDeleted());
                        }
                    }
                });
