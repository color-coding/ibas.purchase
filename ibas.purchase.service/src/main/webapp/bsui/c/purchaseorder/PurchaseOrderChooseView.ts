...
namespace purchase {
    export namespace ui {
        export namespace c {
             ...
             * 选择视图-采购订单
             */
            export class PurchaseOrderChooseView extends ibas.BOQueryDialogViewWithPanel implements app.IPurchaseOrderChooseView {
                /** 返回查询的对象 */
                get queryTarget(): any {
                ...
                draw(): any {
                    ...
                    this.itemTable = new sap.extension.table.DataTable("", {
                        ...
                        nextDataSet(event: sap.ui.base.Event): void {
                            // 查询下一个数据集
                            let data: bo.PurchaseOrderItem = event.getParameter("data");
                            if (ibas.objects.isNull(data)) {
                                return;
                            }
                            if (ibas.objects.isNull(that.lastCriteria)) {
                            ...
                            let docData: bo.PurchaseOrder = new bo.PurchaseOrder();
                        ...
                        rowDoubleClick(event: sap.ui.base.Event): void {
                            let selectItems: bo.PurchaseOrderItem[] = ibas.arrays.create(event.getParameter("row")?.getBindingContext()?.getObject());
                            if (selectItems.length === 0) {
                                selectItems = that.itemTable.getSelecteds();
                            }
                            let selects: bo.PurchaseOrder[] = that.table.getModel().getData("rows");
                            for (let select of selects) {
                                let has: boolean = false;
                                for (let i: number = select.purchaseOrderItems.length - 1; i >= 0; i--) {
                                    let item: any = select.purchaseOrderItems[i];
                                    if (selectItems.find(c => c === item)) {
                                        has = true;
                                        continue;
                                    }
                                    select.purchaseOrderItems.removeAt(i);
                                }
                                if (has === false) {
                                    select.markDeleted();
                                }
                                                    ...
                                                    menu: new sap.m.Menu("", {
                                                        items: [
                                                            this.customMenuItem = new sap.m.MenuItem("", {
                                                                ...
                                                                press: function (): void {
                                                                    let selecteds: ibas.IList<bo.PurchaseOrder> = that.table.getSelecteds();
                                                    ...
                                                    search(event: sap.ui.base.Event): void {
                                                        ...
                                                        if (source instanceof sap.m.SearchField) {
                                                            ...
                                                            let cCriteria: ibas.IChildCriteria = criteria.childCriterias.firstOrDefault(
                                                                c => c.propertyPath === bo.PurchaseOrder.PROPERTY_PURCHASEORDERITEMS_NAME
                                                            );
                ...
                showDataItems(datas: bo.PurchaseOrder[]): void {
                    this.container.to(this.itemTable.getParent().getId());
                    let dataItems: ibas.IList<bo.PurchaseOrderItem> = new ibas.ArrayList<bo.PurchaseOrderItem>();
                    for (let item of datas) {
                        dataItems.add(item.purchaseOrderItems);
                    }
