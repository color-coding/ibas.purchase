...
namespace purchase {
    export namespace ui {
        export namespace c {
             ...
             * 选择视图-采购退货
             */
            export class PurchaseReturnChooseView extends ibas.BOQueryDialogViewWithPanel implements app.IPurchaseReturnChooseView {
                /** 返回查询的对象 */
                get queryTarget(): any {
                ...
                draw(): any {
                    ...
                    this.itemTable = new sap.extension.table.DataTable("", {
                        ...
                        nextDataSet(event: sap.ui.base.Event): void {
                            // 查询下一个数据集
                            let data: bo.PurchaseReturnItem = event.getParameter("data");
                            if (ibas.objects.isNull(data)) {
                                return;
                            }
                            if (ibas.objects.isNull(that.lastCriteria)) {
                            ...
                            let docData: bo.PurchaseReturn = new bo.PurchaseReturn();
                        ...
                        rowDoubleClick(event: sap.ui.base.Event): void {
                            let selectItems: bo.PurchaseReturnItem[] = ibas.arrays.create(event.getParameter("row")?.getBindingContext()?.getObject());
                            if (selectItems.length === 0) {
                                selectItems = that.itemTable.getSelecteds();
                            }
                            let selects: bo.PurchaseReturn[] = that.table.getModel().getData("rows");
                            for (let select of selects) {
                                let has: boolean = false;
                                for (let i: number = select.purchaseReturnItems.length - 1; i >= 0; i--) {
                                    let item: any = select.purchaseReturnItems[i];
                                    if (selectItems.find(c => c === item)) {
                                        has = true;
                                        continue;
                                    }
                                    select.purchaseReturnItems.removeAt(i);
                                }
                                if (has === false) {
                                    select.markDeleted();
                                }
                                                    ...
                                                    menu: new sap.m.Menu("", {
                                                        items: [
                                                            this.customMenuItem = new sap.m.MenuItem("", {
                                                                ...
                                                                press: function (): void {
                                                                    let selecteds: ibas.IList<bo.PurchaseReturn> = that.table.getSelecteds();
                                                    ...
                                                    search(event: sap.ui.base.Event): void {
                                                        ...
                                                        if (source instanceof sap.m.SearchField) {
                                                            ...
                                                            let cCriteria: ibas.IChildCriteria = criteria.childCriterias.firstOrDefault(
                                                                c => c.propertyPath === bo.PurchaseReturn.PROPERTY_PURCHASERETURNITEMS_NAME
                                                            );
                ...
                showDataItems(datas: bo.PurchaseReturn[]): void {
                    this.container.to(this.itemTable.getParent().getId());
                    let dataItems: ibas.IList<bo.PurchaseReturnItem> = new ibas.ArrayList<bo.PurchaseReturnItem>();
                    for (let item of datas) {
                        dataItems.add(item.purchaseReturnItems);
                    }
