...
namespace purchase {
    export namespace ui {
        export namespace c {
             ...
             * 选择视图-采购收货
             */
            export class PurchaseDeliveryChooseView extends ibas.BOQueryDialogViewWithPanel implements app.IPurchaseDeliveryChooseView {
                /** 返回查询的对象 */
                get queryTarget(): any {
                ...
                draw(): any {
                    ...
                    this.itemTable = new sap.extension.table.DataTable("", {
                        ...
                        nextDataSet(event: sap.ui.base.Event): void {
                            // 查询下一个数据集
                            let data: bo.PurchaseDeliveryItem = event.getParameter("data");
                            if (ibas.objects.isNull(data)) {
                                return;
                            }
                            if (ibas.objects.isNull(that.lastCriteria)) {
                            ...
                            let docData: bo.PurchaseDelivery = new bo.PurchaseDelivery();
                        ...
                        rowDoubleClick(event: sap.ui.base.Event): void {
                            let selectItems: bo.PurchaseDeliveryItem[] = ibas.arrays.create(event.getParameter("row")?.getBindingContext()?.getObject());
                            if (selectItems.length === 0) {
                                selectItems = that.itemTable.getSelecteds();
                            }
                            let selects: bo.PurchaseDelivery[] = that.table.getModel().getData("rows");
                            for (let select of selects) {
                                let has: boolean = false;
                                for (let i: number = select.purchaseDeliveryItems.length - 1; i >= 0; i--) {
                                    let item: any = select.purchaseDeliveryItems[i];
                                    if (selectItems.find(c => c === item)) {
                                        has = true;
                                        continue;
                                    }
                                    select.purchaseDeliveryItems.removeAt(i);
                                }
                                if (has === false) {
                                    select.markDeleted();
                                }
                                                    ...
                                                    menu: new sap.m.Menu("", {
                                                        items: [
                                                            this.customMenuItem = new sap.m.MenuItem("", {
                                                                ...
                                                                press: function (): void {
                                                                    let selecteds: ibas.IList<bo.PurchaseDelivery> = that.table.getSelecteds();
                ...
                showDataItems(datas: bo.PurchaseDelivery[]): void {
                    this.container.to(this.itemTable.getParent().getId());
                    let dataItems: ibas.IList<bo.PurchaseDeliveryItem> = new ibas.ArrayList<bo.PurchaseDeliveryItem>();
                    for (let item of datas) {
                        dataItems.add(item.purchaseDeliveryItems);
                    }
