 ...
 *
 */
@LogicContract(IPurchaseDeliveryInvoiceContract.class)
public class PurchaseDeliveryInvoiceService extends PurchaseDeliveryService<IPurchaseDeliveryInvoiceContract> {

	@Override
	protected IPurchaseDelivery fetchBeAffected(IPurchaseDeliveryInvoiceContract contract) {
		return this.fetchBeAffected(contract.getBaseDocumentType(), contract.getBaseDocumentEntry());
	}

	@Override
	protected void impact(IPurchaseDeliveryInvoiceContract contract) {
	...
	@Override
	protected void revoke(IPurchaseDeliveryInvoiceContract contract) {
		if (this.getBeAffected() == null) {
			return;
		}
		IPurchaseDeliveryItem orderItem = this.getBeAffected().getPurchaseDeliveryItems()
