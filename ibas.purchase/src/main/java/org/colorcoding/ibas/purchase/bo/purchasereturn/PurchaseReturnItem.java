	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new IMaterialCatalogCheckContract() {
			...
			public void setCatalogCode(String value) {
				PurchaseReturnItem.this.setCatalogCode(value);
			}
			...
			public String getCatalogCode() {
				return PurchaseReturnItem.this.getCatalogCode();
			}
			...
			public String getBusinessPartnerCode() {
				return PurchaseReturnItem.this.parent.getSupplierCode();
			}
		});
		// 物料发货
		contracts.add(new IMaterialIssueContract() {
			...
			public boolean isOffsetting() {
				...
				if (PurchaseReturnItem.this.parent instanceof IApprovalData) {
					IApprovalData apData = (IApprovalData) PurchaseReturnItem.this.parent;
			...
			public BigDecimal getPrice() {
				...
				return Decimals.divide(PurchaseReturnItem.this.getPreTaxPrice(), PurchaseReturnItem.this.getUOMRate(),
						Decimals.DECIMAL_PLACES_STORAGE);
			}
		...
		if (MyConfiguration.applyVariables(PurchaseOrder.BUSINESS_OBJECT_CODE)
			...
			contracts.add(new IDocumentQuantityReturnContract() {
				...
				@Override
				public boolean checkDataStatus(ITransaction transaction) {
					if (!Strings.isNullOrEmpty(PurchaseReturnItem.this.getBaseDocumentType())) {
						// 基于单据，入废品库，则不统计完成数量
						if (!Strings.isNullOrEmpty(PurchaseReturnItem.this.getWarehouse())) {
					...
					return IDocumentQuantityReturnContract.super.checkDataStatus(transaction);
				}
			});
		} else {
			// 其他基础单据
