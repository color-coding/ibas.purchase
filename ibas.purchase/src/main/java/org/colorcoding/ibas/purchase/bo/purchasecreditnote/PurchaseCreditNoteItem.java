...
@XmlType(name = PurchaseCreditNoteItem.BUSINESS_OBJECT_NAME, namespace = MyConfiguration.NAMESPACE_BO)
public class PurchaseCreditNoteItem extends BusinessObject<PurchaseCreditNoteItem> implements IPurchaseCreditNoteItem,
		IBusinessLogicsHost, IBOTagDeleted, IBOTagCanceled, IBOUserFields, IJECPropertyValueGetter {

	/**
	 * 序列化版本标记
	 */
	private static final long serialVersionUID = -2914081509271315413L;

	/**
	 * 当前类型
	 */
	private static final Class<?> MY_CLASS = PurchaseCreditNoteItem.class;

	/**
	 * 数据库表
	 */
	public static final String DB_TABLE_NAME = "${Company}_PH_RIN1";
	 ...
	 * 业务对象名称
	 */
	public static final String BUSINESS_OBJECT_NAME = "PurchaseCreditNoteItem";
	...
	public void check() throws BusinessRuleException {
		...
		// 基于采购预留发票不执行库存逻辑
		if (MyConfiguration.applyVariables(PurchaseReserveInvoice.BUSINESS_OBJECT_CODE)
				.equalsIgnoreCase(this.getBaseDocumentType())) {
			return;
		}
		// 批次检查
	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new ITaxGroupCheckContract() {
			...
			public String getTax() {
				return PurchaseCreditNoteItem.this.getTax();
			}

			@Override
			public BigDecimal getTaxRate() {
				return PurchaseCreditNoteItem.this.getTaxRate();
			}
		});
		// 物料及仓库检查
		...
		contracts.add(new IMaterialCatalogCheckContract() {
			...
			public void setCatalogCode(String value) {
				PurchaseCreditNoteItem.this.setCatalogCode(value);
			}
			...
			public String getCatalogCode() {
				return PurchaseCreditNoteItem.this.getCatalogCode();
			}
			...
			public String getBusinessPartnerCode() {
				return PurchaseCreditNoteItem.this.parent.getSupplierCode();
			}
		});
		// 基于单据完成数量
		if (!MyConfiguration.applyVariables(PurchaseOrder.BUSINESS_OBJECT_CODE)
			...
			contracts.add(new IDocumentQuantityClosingContract() {
				...
				public IMaterialBatchItems getMaterialBatches() {
					return PurchaseCreditNoteItem.this.getMaterialBatches();
				}

				@Override
				public IMaterialSerialItems getMaterialSerials() {
					return PurchaseCreditNoteItem.this.getMaterialSerials();
				}
		...
		if (!MyConfiguration.applyVariables(PurchaseReturn.BUSINESS_OBJECT_CODE).equals(this.getBaseDocumentType())
			...
			contracts.add(new IMaterialIssueContract() {
				...
				public boolean isOffsetting() {
					...
					if (PurchaseCreditNoteItem.this.parent instanceof IApprovalData) {
						IApprovalData apData = (IApprovalData) PurchaseCreditNoteItem.this.parent;
				...
				public BigDecimal getPrice() {
					...
					return Decimals.divide(PurchaseCreditNoteItem.this.getPreTaxPrice(),
							PurchaseCreditNoteItem.this.getUOMRate(), Decimals.DECIMAL_PLACES_STORAGE);
				}
