	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new IDocumentQuantityClosingContract() {
			...
			public boolean checkDataStatus(ITransaction transaction) {
				if (!Strings.isNullOrEmpty(PurchaseDeliveryItem.this.getBaseDocumentType())) {
					// 基于单据，入废品库，则不统计完成数量
					if (!Strings.isNullOrEmpty(PurchaseDeliveryItem.this.getWarehouse())) {
		...
		contracts.add(new IBlanketAgreementQuantityContract() {
			...
			public BigDecimal getAmount() {
				// 一揽子协议金额按不含税口径
				return PurchaseDeliveryItem.this.getPreTaxLineTotal();
			}
		...
		if (MyConfiguration.getConfigValue(MyConfiguration.CONFIG_ITEM_ENABLE_MATERIAL_PURCHASE_PRICE_RECORDING,
			...
			contracts.add(new IMaterialPriceContract() {
				...
				public BigDecimal getPrice() {
					...
					return Decimals.multiply(PurchaseDeliveryItem.this.getPreTaxPrice(), rate);
				}
			});
		}
